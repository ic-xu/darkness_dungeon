...
class ConnectionManager with ChangeNotifier {
  ...
  Utf8Codec utf8codec = new convert.Utf8Codec();
  ...
  configure(String url) async {
    serverHost = url;
    login(url, "testuser", "passwd");
  }

  _connectStatusChange(bool value) {
    connectStatus = value;
    if (!connectStatus) {
      _singleton = null;
    }
  ...
  Future<bool> login(String socketUrl, String username, String password) async {
    try {
      ...
      client.subscribe("/game/join", MqttQos.atLeastOnce);
  ...
  static _onMessage(MqttCustomerMessage message) {
    ...
    Map map = convert.jsonDecode(messageString);
    mainEventBus.streamController.add(map);
    // notifyListeners();
  }

  _onPublishMessage(MqttPublishMessage mqttPublishMessage) {
    mainEventBus.streamController.add(mqttPublishMessage);
  }
  ...
  void _onUdpMessage(List<int> message) {
    try{
      ...
      message.removeAt(0);
      var msgString = utf8.decoder.convert(message);
      var data = json.decode(msgString);
      // print(msgString);
      mainEventBus.streamController.add(data);
    }catch (e){
      print(e);
    }
  }
